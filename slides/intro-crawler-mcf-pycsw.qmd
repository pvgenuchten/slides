...
- Metadata should own metadata records, because they are the knowledgable party.
- So, stay close to the tools that they already use: Python, Git, Excel.
...
- Builds on a convention of placing a README.md file in a project folder with source and result data of a model run, describing the source, attribution, date of the source data. 
...
- A [crawler tool](https://pypi.org/project/geodatacrawler/) fetches from a set of project folders the mcf files and stores it in a central index, [pycsw](https://pycsw.org)
...
- a generic query to a remote catalogue endpoint
- a excel sheet of records (each column is a metadata property)
- a list of DOI's

A challenge with existing data portals is the lack of user interaction, users often can't flag problems in metadata or suggest new content

This approach improves that by storing the MCF in GIT, and add a 'edit me on git' link on each metadata page, also the harvest definition is stored on git
